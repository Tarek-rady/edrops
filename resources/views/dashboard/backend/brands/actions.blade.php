<td>
    <div class="hstack gap-3 flex-wrap">
        @if(auth('admin')->user()->hasPermission('brands-update'))
          <a href="{{ route('admin.brands.edit' , $id) }}" class="link-success fs-15 icon1"><i class="ri-edit-2-line"></i></a>
        @endif
...
@php
    $brand = App\Models\Category::where('id' , $id)->first();
@endphp


<!-- Modal -->
    <div class="modal fade zoomIn" id="deleteRecordModal{{ $id }}" tabindex="-1" aria-hidden="true">
        <form action="{{ route('admin.brands.destroy' , $id) }}" method="POST">
            @method('DELETE')
            @csrf
                    ...
                    <div class="modal-body">
                        <div class="mt-2 text-center">
                            ...
                            <div class="mt-4 pt-2 fs-15 mx-4 mx-sm-5">
                                ...
                                <p class="text-muted mx-4 mb-0">{{ $brand->name }}</p>
                            </div>
                        </div>
