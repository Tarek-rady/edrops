<td>
    <div class="hstack gap-3 flex-wrap">
        @if(auth('admin')->user()->hasPermission('asks-update'))
          <a href="{{ route('admin.asks.edit' , $id) }}" class="link-success fs-15 icon1"><i class="ri-edit-2-line"></i></a>
        @endif
        @if(auth('admin')->user()->hasPermission('asks-delete'))
...
<!-- Modal -->
    <div class="modal fade zoomIn" id="deleteRecordModal{{ $id }}" tabindex="-1" aria-hidden="true">
        <form action="{{ route('admin.asks.destroy' , $id) }}" method="POST">
            @method('DELETE')
            @csrf
